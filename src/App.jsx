...
function SpendingExplorer({ departments, spendingOverTime, initialYear }) {
  ...
  return (
    <div className="section">
      ...
      {selectedDept && (
        <div ref={detailPanelRef} className="chart-card" style={{ marginBottom: 24, border: '1px solid var(--accent-red)', position: 'relative' }}>
...
function VendorExplorer({ spendingYear }) {
  ...
  useEffect(() => {
    ...
    if (nonProfitFilter) {
      fetchNonProfitVendors(vendorYear, 200).then(data => {
        setVendors(data);
  ...
  return (
    <div className="section">
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        <button
          onClick={() => setNonProfitFilter(!nonProfitFilter)}
          style={{
            padding: '12px 16px', borderRadius: 8, border: '1px solid var(--border)',
            background: nonProfitFilter ? 'rgba(255,170,34,0.2)' : 'var(--bg-secondary)',
            color: nonProfitFilter ? 'var(--accent-gold)' : 'var(--text-secondary)',
            fontSize: '0.85rem', fontWeight: nonProfitFilter ? 600 : 400,
            cursor: 'pointer', whiteSpace: 'nowrap', transition: 'all 0.2s',
          }}
        >
          {nonProfitFilter ? 'Non-Profits Only' : 'All Vendors'}
        </button>
      ...
      {selectedVendor && (
        <div ref={vendorDetailPanelRef} className="chart-card" style={{ marginBottom: 24, border: '1px solid var(--accent-gold)', position: 'relative' }}>
...
function QuasiExplorer({ quasiPayments }) {
  ...
  return (
    <div className="section">
      ...
      {/* Agency Detail Panel */}
      {selectedAgency && (
        <div ref={quasiDetailPanelRef} className="chart-card" style={{ marginBottom: 24, border: '1px solid var(--accent-red)', position: 'relative' }}>
          <button onClick={() => { setSelectedAgency(null); setAgencyDetail(null); }} style={{
            position: 'absolute', top: 12, right: 12, background: 'var(--bg-secondary)',
            border: '1px solid var(--border)', borderRadius: 6, color: 'var(--text-secondary)',
            padding: '4px 12px', cursor: 'pointer', fontSize: '0.8rem',
          }}>Close</button>

          <h3 style={{ color: 'var(--accent-red)', marginBottom: 4 }}>{selectedAgency}</h3>
          <div className="chart-subtitle">Complete spending breakdown — every vendor, every dollar</div>

          {detailLoading ? (
            <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)' }}>Loading agency data...</div>
          ) : agencyDetail && (
            <>
              {/* Spending over time */}
              {agencyDetail.byYear.length > 0 && (
                <div style={{ marginTop: 20 }}>
                  ...
                  <span style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>
                    All-time total: <strong style={{ color: 'var(--accent-red)' }}>{formatMoney(agencyDetail.byYear.reduce((s, y) => s + y.total, 0))}</strong>
                  </span>
                </div>
              )}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: 20, marginTop: 24 }}>
                ...
                {agencyDetail.categories.length > 0 && (
                  <div>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px 16px', fontSize: '0.75rem' }}>
                      {agencyDetail.categories.slice(0, 8).map((c, i) => (
                        <span key={i} style={{ color: COLORS[i % COLORS.length] }}>
                ...
                <div style={{ marginTop: 24 }}>
                  ...
                  <div className="data-table-wrapper">
                    <table className="data-table">
                      <thead>
                        <tr>
                          <th>Date</th><th>Amount</th><th>Vendor</th><th>Account</th><th>Department</th><th>Category</th>
                        </tr>
                      </thead>
                      <tbody>
                  ...
                  {agencyDetail.payments.length > PAYMENTS_PER_PAGE && (
                    <div style={{ display: 'flex', gap: 8, justifyContent: 'center', marginTop: 12 }}>
                      ...
                      <span style={{ padding: '6px 12px', color: 'var(--text-muted)', fontSize: '0.85rem' }}>
                        Page {paymentPage + 1} of {Math.ceil(agencyDetail.payments.length / PAYMENTS_PER_PAGE)}
                      </span>
                      <button disabled={(paymentPage + 1) * PAYMENTS_PER_PAGE >= agencyDetail.payments.length}
                        onClick={() => setPaymentPage(p => p + 1)} style={{
                        padding: '6px 16px', borderRadius: 6, border: '1px solid var(--border)',
                        background: 'var(--bg-secondary)', color: 'var(--text-primary)',
                        cursor: (paymentPage + 1) * PAYMENTS_PER_PAGE >= agencyDetail.payments.length ? 'not-allowed' : 'pointer',
                        opacity: (paymentPage + 1) * PAYMENTS_PER_PAGE >= agencyDetail.payments.length ? 0.4 : 1,
                      }}>Next</button>
                    </div>
                  )}
                </div>
          ...
          <div className="chart-card">
            ...
            <ResponsiveContainer width="100%" height={600}>
              <BarChart data={quasiPayments.slice(0, 20)} layout="vertical" margin={{ left: 220 }}>
                ...
                <Tooltip content={<CustomTooltip />} />
                <Bar dataKey="value" fill="#ff6644" radius={[0, 6, 6, 0]} name="Total Payments" cursor="pointer"
      ...
      <div style={{ marginTop: 32 }}>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
          ...
          >
            ...
            <option value="department">Search by Agency</option>
          </select>
        </div>
            ...
            {searchLoading ? (
              ...
              <>
                ...
                {searchResults.length > RESULTS_PER_PAGE && (
                  <div style={{ display: 'flex', gap: 8, justifyContent: 'center', marginTop: 12 }}>
                    <button disabled={searchPage === 0} onClick={() => setSearchPage(p => p - 1)} style={{
                      padding: '6px 16px', borderRadius: 6, border: '1px solid var(--border)',
                      background: 'var(--bg-secondary)', color: 'var(--text-primary)',
                      cursor: searchPage === 0 ? 'not-allowed' : 'pointer', opacity: searchPage === 0 ? 0.4 : 1,
                    }}>Previous</button>
                    <span style={{ padding: '6px 12px', color: 'var(--text-muted)', fontSize: '0.85rem' }}>
                      Page {searchPage + 1} of {Math.ceil(searchResults.length / RESULTS_PER_PAGE)}
                    </span>
                    <button disabled={(searchPage + 1) * RESULTS_PER_PAGE >= searchResults.length}
                      onClick={() => setSearchPage(p => p + 1)} style={{
                      padding: '6px 16px', borderRadius: 6, border: '1px solid var(--border)',
                      background: 'var(--bg-secondary)', color: 'var(--text-primary)',
                      cursor: (searchPage + 1) * RESULTS_PER_PAGE >= searchResults.length ? 'not-allowed' : 'pointer',
                      opacity: (searchPage + 1) * RESULTS_PER_PAGE >= searchResults.length ? 0.4 : 1,
                    }}>Next</button>
                  </div>
                )}
              </>
...
export default function App() {
  ...
  return (
    <>
      ...
      {/* ============ PAYROLL ============ */}
      {activeSection === 'payroll' && <PayrollSearcher payrollYear={payrollYear} setPayrollYear={setPayrollYear} data={data} />}
