...
import { Search, TrendingUp, DollarSign, FileText, AlertCircle, Loader, ChevronRight, Users, Activity, Award } from 'lucide-react';
import {
...
const COLORS = ['#4361ee', '#e76f51', '#2a9d8f', '#e9c46a', '#264653', '#7209b7', '#f4845f', '#577590', '#c77dff', '#6c8eb5'];
...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{
      ...
      {payload.map((p, i) => (
        <p key={i} style={{ color: p.color || '#4361ee' }}>
          {p.name}: {typeof p.value === 'number' ? formatMoneyFull(p.value) : p.value}
        </p>
      ))}
    </div>
  );
...
export default function OcpfDataCenter() {
  ...
  // Load top candidates for selected year
  useEffect(() => {
    setTopLoading(true);
    fetchLegislatorFinances(parseInt(topYear), topYear % 2 === 0 ? 'state' : 'municipal').then(data => {
      const arr = Array.isArray(data) ? data : (data?.data || []);
      const sorted = [...arr].sort((a, b) => (b.receipts || 0) - (a.receipts || 0));
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      // Search across multiple years of depository reports
      const years = [2024, 2025, 2022, 2023];
      let allResults = [];
      for (const yr of years) {
        const raceType = yr % 2 === 0 ? 'state' : 'municipal';
        const data = await fetchLegislatorFinances(yr, raceType);
        const arr = Array.isArray(data) ? data : (data?.data || []);
        const matches = arr.filter(leg =>
          leg.name.toLowerCase().includes(searchText.toLowerCase())
        );
        matches.forEach(m => {
          if (!allResults.find(r => r.cpfId === m.cpfId)) {
            allResults.push({ ...m, year: yr });
          }
        });
        if (allResults.length >= 5) break;
      }
      // If still nothing, try contribution search
      if (allResults.length === 0) {
        ...
        allResults = Object.values(recipientMap);
      }
  ...
  // Load detail data when politician selected
  useEffect(() => {
    ...
    ]).then(([contribData, expendData]) => {
      setContributions(contribData?.items || []);
      setExpenditures(expendData?.items || []);
  ...
  const SortHeader = ({ label, sortKey, asc, desc, currentSort, setSort }) => {
  ...
  return (
    <div>
        ...
        {ytdLoading ? (
          ...
          <div className="kpi-row" style={{ marginBottom: 0 }}>
            <div className="kpi-card" style={{ borderColor: 'rgba(67,97,238,0.3)' }}>
              ...
              <div className="kpi-value" style={{ color: '#4361ee', fontSize: '1.8rem' }}>
                {(ytdStats.contributionCount || 0).toLocaleString()}
              </div>
            ...
            <div className="kpi-card" style={{ borderColor: 'rgba(231,111,81,0.3)' }}>
              ...
              <div className="kpi-value" style={{ color: '#e76f51', fontSize: '1.8rem' }}>
                {(ytdStats.expenditureCount || 0).toLocaleString()}
              </div>
            ...
            <div className="kpi-card" style={{ borderColor: 'rgba(42,157,143,0.3)' }}>
              ...
              <div className="kpi-value" style={{ color: '#2a9d8f', fontSize: '1.8rem' }}>
                {(ytdStats.reportCount || 0).toLocaleString()}
              </div>
      ...
      {/* ===== TOP FUNDED CANDIDATES BAR CHART ===== */}
      <div className="card" style={{ padding: 24, marginBottom: 32, background: 'var(--bg-card)', borderRadius: 16, border: '1px solid var(--border)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div>
            ...
            <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
              {topYear % 2 === 0 ? 'State' : 'Municipal'} race depository reports — ranked by total receipts
            </p>
          </div>
          <select
            value={topYear}
        ...
        {topLoading ? (
          <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)' }}>
            <div className="spinner" style={{ margin: '0 auto 8px' }} /> Loading candidates...
          </div>
        ) : topCandidates.length === 0 ? (
          <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)' }}>
            No depository reports found for {topYear}.
          </div>
        ) : (
          <>
            <ResponsiveContainer width="100%" height={Math.min(400, topCandidates.slice(0, 15).length * 36 + 40)}>
              <BarChart
              ...
              >
                ...
                <Bar dataKey="receipts" radius={[0, 6, 6, 0]}>
                  {topCandidates.slice(0, 15).map((_, i) => (
      ...
      <div className="card" style={{ padding: 24, marginBottom: 32, background: 'var(--bg-card)', borderRadius: 16, border: '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <div style={{ position: 'relative', flex: 1 }}>
            ...
            <input
              ...
              placeholder="Enter politician name (e.g. Healey, Spilka, Mariano...)"
              value={searchText}
        ...
        {searchResults.length > 0 && !selectedPolitician && (
          <div style={{ marginTop: 16 }}>
            <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: 8 }}>
              {searchResults.length} result(s) found — click to view details:
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {searchResults.map(leg => (
                <button
                  key={leg.cpfId}
                  onClick={() => { setSelectedPolitician(leg); setDetailTab('contributions'); }}
                  style={{
        ...
        {searchResults.length === 0 && searchText && !searching && !selectedPolitician && (
          <p style={{ marginTop: 12, fontSize: '0.85rem', color: 'var(--text-muted)' }}>
            No politicians found matching "{searchText}". Try a different name.
          </p>
        )}
      </div>

      {/* ===== POLITICIAN DETAIL PANEL ===== */}
      {selectedPolitician && (
        <div className="card" style={{ padding: 24, background: 'var(--bg-card)', borderRadius: 16, border: '1px solid var(--border)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            ...
            <button
              onClick={() => { setSelectedPolitician(null); setContributions([]); setExpenditures([]); }}
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <ResponsiveContainer width="100%" height={220}>
                <LineChart data={contribByMonth}>
                  ...
                  <Line type="monotone" dataKey="amount" stroke="#2a9d8f" strokeWidth={2} dot={{ fill: '#2a9d8f', r: 3 }} name="Amount" />
                </LineChart>
              </ResponsiveContainer>
            </div>
          )}
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: 6, transition: 'all 0.2s',
                }}
              >
          ...
          {detailLoading ? (
            <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)' }}>
              <div className="spinner" style={{ margin: '0 auto 8px' }} /> Loading campaign finance data...
            </div>
          ) : detailTab === 'contributions' ? (
            <div>
              {sortedContributions.length === 0 ? (
                <p style={{ padding: 20, textAlign: 'center', color: 'var(--text-muted)' }}>No contributions found for this politician.</p>
              ) : (
            ...
            <div>
              {sortedExpenditures.length === 0 ? (
                <p style={{ padding: 20, textAlign: 'center', color: 'var(--text-muted)' }}>No expenditures found for this politician.</p>
              ) : (
                <div className="data-table-wrapper">
                  <table className="data-table">
                    <thead>
