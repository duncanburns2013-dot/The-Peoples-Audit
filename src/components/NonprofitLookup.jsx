...
export default function NonprofitLookup() {
  ...
  const handleSelectOrg = useCallback(async (ein) => {
    ...
    try {
      ...
      if (data.organization) {
        // Merge filings with and without extracted financial data
        const withData = (data.filings_with_data || []).map(f => ({ ...f, _hasData: true }));
        const withoutData = (data.filings_without_data || []).map(f => ({ ...f, _hasData: false }));
        // Deduplicate by tax period (prefer filings with data)
        const seen = new Set(withData.map(f => f.tax_prd));
        const unique = [...withData, ...withoutData.filter(f => !seen.has(f.tax_prd))];
        // Sort by year descending (newest first)
        unique.sort((a, b) => (b.tax_prd_yr || 0) - (a.tax_prd_yr || 0));
        setOrgDetails({
          ...data.organization,
          filings: unique,
        });
  ...
  const getFilingsData = () => {
    ...
    return orgDetails.filings
      .filter((f) => f._hasData !== false && (f.totrevenue || f.totfuncexpns || f.totexpense))
      .slice(0, 10)
      .reverse()
      .map((filing) => ({
            ...
            <div className="chart-card" style={{ marginTop: 12 }}>
              ...
              <div style={{ overflowX: 'auto' }}>
                <table className="data-table" style={{ width: '100%' }}>
                  ...
                  <tbody>
                    {orgDetails.filings.slice(0, 10).map((filing, idx) => (
                      <tr key={idx}>
                        <td style={{ fontWeight: 600 }}>{filing.tax_prd_yr || filing.tax_year || filing.year}</td>
                        <td style={{ color: 'var(--accent-green)', fontWeight: 600 }}>{filing.totrevenue != null ? formatMoney(filing.totrevenue) : '—'}</td>
                        <td style={{ color: '#E67E22', fontWeight: 600 }}>{(filing.totfuncexpns || filing.totexpense) != null ? formatMoney(filing.totfuncexpns || filing.totexpense) : '—'}</td>
                        <td>
                          {filing.pdf_url ? (
